...
const MapBox: React.FC<MapboxMapProps> = ({
  ...
  const [completedPolygon, setCompletedPolygon] = useState<number[][] | null>(
    null
  );
  ...
  const filterAgentData = (
    ...
    return agents.filter((agent) => {
      const stateMatch =
        currentFilters.stateLocation.length === 0 ||
        currentFilters.stateLocation.includes(
          agent["State/Location"]?.toString().trim() || ""
        );

      const divisionMatch =
        currentFilters.division.length === 0 ||
        currentFilters.division.includes(
          agent.Division?.toString().trim() || ""
        );

      const departmentMatch =
        currentFilters.department.length === 0 ||
        currentFilters.department.includes(
          agent.Department?.toString().trim() || ""
        );

      const managerMatch =
        currentFilters.managerName.length === 0 ||
        currentFilters.managerName.includes(
          agent["Manager Name"]?.toString().trim() || ""
        );
  ...
  useEffect(() => {
    const fetchAgentData = async () => {
      try {
        ...
        if (result.data) {
          const validAgents = result.data.filter(
            (agent: AgentData) =>
              agent.latitude &&
              agent.longitude &&
  ...
  const updateDataSources = () => {
    if (!map.current || !mapLoaded || !dataLoaded || agentData.length === 0)
      return;
    ...
    console.log(
      `Updating map with ${filteredAgents.length} filtered agents out of ${agentData.length} total`
    );

    if (map.current.getSource("agents")) {
      (map.current.getSource("agents") as mapboxgl.GeoJSONSource).setData(
        geoJsonData as any
      );
    }
    ...
    if (map.current.getSource("agents-heatmap")) {
      (
        map.current.getSource("agents-heatmap") as mapboxgl.GeoJSONSource
  ...
  const addHeatmapLayer = () => {
    ...
    console.log(
      `Added heatmap layer with ${filteredAgents.length} filtered agents`
    );
  };
    ...
    if (!eventListenersAddedRef.current) {
      if (enableClustering) {
        map.current.on("click", "clusters", (e) => {
          ...
          (
            map.current!.getSource("agents") as mapboxgl.GeoJSONSource
